-- Update function to support search by order number in addition to phone search
...
BEGIN
  ...
  -- Try to parse as order number (if it's all digits and not a phone number)
  IF digits_only ~ '^[0-9]+$' AND NOT is_full_number THEN
    search_order_number := digits_only::INTEGER;
  ELSE
    search_order_number := NULL;
  END IF;
  ...
  WHERE
    -- Search by order number (exact match or partial)
    (search_order_number IS NOT NULL AND (
      o.order_number = search_order_number OR
      o.order_number::text LIKE '%' || digits_only || '%'
    ))
    OR
    -- Phone number search logic (existing functionality)
    CASE 
